"""Extract Dubois — *Perfectionnement : les combinaisons* into the interactive
ManuelInteractif reader (prose + playable boards).

This book is a text PDF in the proven Dubois "D-format": chapters of prose, then
exercise pages (diagrams D1..Dn with "trait aux blancs/noirs") paired with a
SOLUTIONS page. Positions are men-only combinations, so the pixel FEN extractor
reads them exactly; solutions are the book's *verbatim* published notation,
replayed through the engine and kept only when the whole line is legal (so a
mis-read board or a variation-laden line is simply not claimed as solvable —
nothing is invented).

Run (from repo root, with backend on PYTHONPATH)::

    PYTHONPATH=backend python scripts/book_extraction/build_perfectionnement_combinaisons.py

Output: frontend/src/manuels/data/perfectionnement_combinaisons.ts
"""
...
PDF = _ROOT / "docs/livres/perfectionnement/dubois_perfectionnement_combinaisons.pdf"
OUT = _ROOT / "frontend/src/manuels/data/perfectionnement_combinaisons.ts"
...
def _reconstruct(fen: str, tokens: list[str]) -> list[dict]:
    st = ge.fen_to_board(fen)
    out: list[dict] = []
    for pdn in tokens:
...
def main() -> int:
    ...
    text = [doc[i].get_text() for i in range(n_pages)]
    flat = [t.replace("\n", " ") for t in text]

    # Chapters: page → (num, title). Title is the "Chapitre N : …" line.
    chapters_raw = []  # (page, num, title)
    for i in range(n_pages):
        m = re.search(r"Chapitre\s+(\d+)\s*:\s*([^\n]+)", text[i])
        if m and "..." not in m.group(2):  # skip the table of contents dotted lines
            title = f"Chapitre {m.group(1)} : {m.group(2).strip()}"
            chapters_raw.append((i + 1, int(m.group(1)), title))
    # Keep first occurrence of each chapter number, in order.
    seen = set()
    chapters_raw = [c for c in chapters_raw if not (c[1] in seen or seen.add(c[1]))]

    # Exercise/solution blocks: each SOLUTIONS page preceded by 1-2 diagram pages.
    def dtraits(page: int):
        return re.findall(r"D(\d+)\s*:\s*trait aux (blancs|noirs)", flat[page - 1])
    sol_pages = [i + 1 for i in range(n_pages) if "SOLUTIONS" in text[i]]
    blocks = []
    for s in sol_pages:
    ...
    # Assign each block to its chapter (the last chapter starting at or before it).
    def chapter_of(page: int) -> int:
        num = chapters_raw[0][1]
        for (cp, cnum, _t) in chapters_raw:
            if cp <= page:
                num = cnum
            else:
                break
    ...
    for expgs, s in blocks:
        ...
        # Parse the solutions page: D<n> -> its text block.
        ...
        for p in expgs:
            ...
            for (x1, y1, x2, y2) in find_boards(g, "border_lines", 400, 505):
                ...
                if len(moves) >= 2:
                    ...
                    n_solv += 1
                    # Prose comment (attribution / idea), sans the move line.
    ...
    # Assemble reader: preface + each chapter (prose then its boards).
    chapters = [{"n": 0, "title": "Présentation"}]
    blocks_out = [
        {"type": "h1", "ch": 0, "runs": [{"t": "Perfectionnement — Les combinaisons"}]},
        {"type": "p", "ch": 0, "runs": [{"t": "J-P. Dubois. Combinaisons avancées : "
            "chaque chapitre expose un thème, puis des diagrammes à jouer ou à résoudre."}]},
    ]
    # Chapter prose: text from the chapter's start page up to the next chapter.
    chap_pages = {c[1]: c[0] for c in chapters_raw}
    ...
    for idx, (cp, cnum, title) in enumerate(ordered):
        ...
        prose = []
        for pg in range(cp, min(end, cp + 2)):  # intro prose lives on the first pages
    ...
    data = {"book": "Dubois — Perfectionnement : les combinaisons",
            "level": "Avancé", "chapters": chapters,
            "blocks": blocks_out, "positions": positions}

    header = ("// Auto-generated by scripts/book_extraction/"
              "build_perfectionnement_combinaisons.py — do not edit by hand.\n"
    ...
    OUT.write_text(header + f"const DATA: ManuelData = {json.dumps(data, ensure_ascii=False, indent=0)}\n\nexport default DATA\n",
                   encoding="utf-8")
    print(f"chapters={len(chapters)}  diagrams={n_diag}  solvable={n_solv}  "
