...
export default function App() {
  ...
  // Curriculum module opened as an interactive reader (lessons = chapters).
  const [parcoursModuleId, setParcoursModuleId] = useState<string | null>(null)
  ...
  const [explorerMaxMoves, setExplorerMaxMoves] = useState<number>(() => {
    try { return Number(localStorage.getItem('explorerMaxMoves') ?? '12') || 12 } catch { return 12 }
  })
  ...
  return (
    <div className="bg-gray-900 text-gray-100 flex flex-col h-full">
      ...
      <main className="flex-1 overflow-hidden lg:overflow-y-auto relative">
        ...
        {/* PARCOURS MODULE READER — a learning-path module opened as the
            interactive reader (its lessons become chapters: prose + playable
            boards), generated by curriculum/build_parcours_manuels.py. */}
        {tab === 'parcours-manuel' && parcoursModuleId && (
          <ManuelInteractifPage
            parcoursId={parcoursModuleId}
            onClose={() => setTab('learning-path')}
          />
        )}
        ...
        {/* INTERACTIVE MANUALS TAB — library + self-contained reader for the
