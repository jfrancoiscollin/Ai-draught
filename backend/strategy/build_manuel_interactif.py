...
from strategy.steppable_lines import insert_embedded_games
...
def build_source(source: str) -> dict:
    ...
    # Replace the game notation printed inside the prose (main line, variations
    # in parentheses) with a steppable board, keeping the commentary. Engine-
    # verified: only games whose whole main line replays from the initial
    # position are converted.
    data["blocks"], n = insert_embedded_games(
        data["blocks"], data["positions"], SOURCES[source]["slug"])
    if n:
        print(f"  ({source}: {n} parties rendues défilables)")
    return data
